...
Arduino programs consist of two parts. setup(), which will only be ran once, and loop() which will run forever. In the setup part, the redboard constructs the nercessary variables for MAX_BOARDS number of boards. Then it sets up the serial interface, and waits for a ready signal from the computer. Then it starts listening for a json file. 
...
The boards are then confugured. 

One loop goes as follows. The redboard listens for a ready signal. When it arrives, it sets the motors in the current position. Then it increments the position variable. 

#Redboard side
Client is the program that runs on the user side. 
The user can modify the configuration data that will be sent as a json sting to the redboard in getConfig in MCutils.py. 
