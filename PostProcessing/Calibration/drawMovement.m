...
figure;
scatter3(from(:,2),from(:,3),from(:,4),'b','f');
hold on;
...
for i = 2:length(ordered)
    pts = ordered{i}.all;
    scatter3(pts(:,2),pts(:,3),pts(:,4),'b','f');
    scatter3(pts(ordered{i}.estimated,2),pts(ordered{i}.estimated,3),pts(ordered{i}.estimated,4),'r','f');
    
    move = pts(from(:,1),:) - from;
    scale = 0;
